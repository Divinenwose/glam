          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-700">
              {mockPayments.map((payment) => (
                <tr key={payment.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800">
                  <td className="px-6 py-4 font-mono text-sm text-neutral-900 dark:text-white">{payment.id}</td>
                  <td className="px-6 py-4 text-neutral-600 dark:text-neutral-400 capitalize">{payment.type.replace('_', ' ')}</td>
                  <td className="px-6 py-4 text-neutral-900 dark:text-white">{payment.user}</td>
                  <td className="px-6 py-4 font-medium text-neutral-900 dark:text-white">{formatCurrency(payment.amount)}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${
                      payment.status === 'completed' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' :
                      payment.status === 'pending' ? 'bg-gold-100 dark:bg-gold-900/30 text-gold-700 dark:text-gold-400' :
                      'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                    }`}>
                      {payment.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-neutral-600 dark:text-neutral-400">{payment.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
